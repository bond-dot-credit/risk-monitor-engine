...
/**
 * Complete NEAR Protocol Rewards Executor
 * 
...
async function executeUniqueWalletTransactions(wallets: DerivedWallet[], nodeUrl: string): Promise<number> {
  ...
  for (let i = 0; i < wallets.length; i += batchSize) {
    ...
    // Execute one view call from each account in this batch (counts toward unique wallets)
    for (const account of accounts) {
      try {
        // Execute a view function call from this account
        await (account as any).viewFunction({
        ...
        successfulWallets++;
        console.log(`   ✅ View call successful from ${account.accountId}! (Total unique wallets: ${successfulWallets})`);
      } catch (error) {
        console.error(`   ❌ View call failed from ${account.accountId}:`, error);
      }
    }
...
async function executeCompleteProtocolRewards() {
  console.log('🚀 NEAR Protocol Rewards - Complete Executor');
  console.log('========================================');
  ...
  try {
    ...
    // 1. Execute transactions to increase volume
    console.log('\n1️⃣  Increasing Transaction Volume');
    const volumeTransactions = await executeVolumeTransactions(mainAccount, 100);
    
    // 2. Execute smart contract calls (we already did 500, but let's do a few more for safety)
    console.log('\n2️⃣  Increasing Smart Contract Calls');
    const smartContractCalls = await executeSmartContractCalls(mainAccount, 50);
    
    // 3. Execute transactions from unique wallets
    ...
    // Summary
    console.log('\n🎉 Complete Protocol Rewards Execution Completed!');
    console.log('==============================================');
    ...
    if (estimatedVolumeUSD >= 10000) {
      ...
      const neededVolume = (10000 - estimatedVolumeUSD) / 2.5 / 0.001;
      console.log(`\n📊 You need ${Math.ceil(neededVolume)} more transactions to reach the $10,000+ volume requirement`);
    }
    ...
    console.error('❌ Error executing complete protocol rewards:', error);
...
if (require.main === module) {
  executeCompleteProtocolRewards().catch(console.error);
}