...
export function calculatePerceptionScore(agent: Agent, reputationEvents?: ReputationEvent[]): number {
  let score = 0;
  let maxScore = 0;

  // Community trust (40 points)
  const communityTrustScore = calculateCommunityTrustScore(agent, reputationEvents);
  score += communityTrustScore * 0.40;
  maxScore += 40;

  // Reputation metrics (35 points)
  const reputationScore = calculateReputationMetricsScore(agent, reputationEvents);
  score += reputationScore * 0.35;
  maxScore += 35;

  // Social proof (25 points)
  const socialProofScore = calculateSocialProofScore(agent);
  score += socialProofScore * 0.25;
  maxScore += 25;

  return Math.round((score / maxScore) * 100);
}

function calculateCommunityTrustScore(agent: Agent, reputationEvents?: ReputationEvent[]): number {
  let score = 0;

  if (reputationEvents && reputationEvents.length > 0) {
    // Positive vs negative events ratio
    const positiveEvents = reputationEvents.filter(e => e.impact > 0);
    const negativeEvents = reputationEvents.filter(e => e.impact < 0);
    
    if (positiveEvents.length > 0) {
      const trustRatio = positiveEvents.length / (positiveEvents.length + negativeEvents.length);
      score += trustRatio * 25;
    }

    // Recent positive events
    const recentPositiveEvents = reputationEvents
      .filter(e => e.impact > 0 && new Date(e.timestamp).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000);
    score += Math.min(15, recentPositiveEvents.length * 3);
  }

  return Math.min(40, score);
}

function calculateReputationMetricsScore(agent: Agent, reputationEvents?: ReputationEvent[]): number {
  let score = 0;

  // Overall agent score as reputation indicator
  score += (agent.score.overall / 100) * 20;
...
function calculateSocialProofScore(agent: Agent): number {
  let score = 0;

  // Verification methods as social proof
  const verificationMethods = agent.metadata.verificationMethods || [];
  const passedVerifications = verificationMethods.filter(m => m.status === VerificationStatus.PASSED);
  
  score += Math.min(15, passedVerifications.length * 3);

  // Audit reports and external validation
  if (agent.metadata.provenance.auditReport) score += 5;
  if (agent.metadata.provenance.auditScore && agent.metadata.provenance.auditScore > 80) score += 5;
...
export function calculateLTV(baseLTV: number, agentScore: AgentScore, adjustments: LTVAdjustment[] = [], collateral: Collateral[] = []): LTVCalculation {
  ...
  adjustments.forEach(adjustment => {
    switch (adjustment.type) {
      ...
      case 'confidence_bonus':
        if (agentScore.confidence >= 85) {
          finalLTV += 2;
          confidence += 0.06;
        } else if (agentScore.confidence >= 75) {
          finalLTV += 1;
          confidence += 0.03;
        }
        break;
      case 'performance_bonus':
        if (agentScore.performance >= 90) {
          finalLTV += 2;
          confidence += 0.05;
        } else if (agentScore.performance >= 80) {
          finalLTV += 1;
          confidence += 0.03;
        }
        break;
...
export function buildReputationSummary(agentId: string, events: ReputationEvent[]): ReputationSummary {
  if (events.length === 0) {
    return {
      agentId,
      lastUpdated: new Date(),
      ...
      positiveEvents: 0,
      negativeEvents: 0,
      breakdown: { performance: 0, credit: 0, risk: 0, compliance: 0, overall: 0 },
      recentEvents: [],
      trend: 'stable'
    };
  }
  ...
  const trend = determineReputationTrend(events);
...
function calculateReputationBreakdown(events: ReputationEvent[]): ReputationSummaryBreakdown {
  const performanceEvents = events.filter(e => 
    e.type.includes('PERFORMANCE') || e.type.includes('APR') || e.type.includes('AUM')
  );
  const creditEvents = events.filter(e => 
    e.type.includes('CREDIT') || e.type.includes('LTV')
  );
  const riskEvents = events.filter(e => 
    e.type.includes('RISK') || e.type.includes('VOLATILITY')
  );
  const complianceEvents = events.filter(e => 
    e.type.includes('COMPLIANCE')
  );

  const calculateEventScore = (eventList: ReputationEvent[]) => {
    if (eventList.length === 0) return 0;
    const totalImpact = eventList.reduce((sum, e) => sum + e.impact, 0);
    return Math.max(0, Math.min(100, 50 + (totalImpact / eventList.length)));
  };

  const performance = calculateEventScore(performanceEvents);
  const credit = calculateEventScore(creditEvents);
  const risk = calculateEventScore(riskEvents);
  const compliance = calculateEventScore(complianceEvents);
  const overall = Math.round((performance + credit + risk + compliance) / 4);

  return { performance, credit, risk, compliance, overall };
}
...
function determineReputationTrend(events: ReputationEvent[]): 'improving' | 'stable' | 'declining' {
  ...
  const olderImpact = olderEvents.reduce((sum, e) => sum + e.impact, 0);

  if (recentImpact > olderImpact + 10) return 'improving';
  if (recentImpact < olderImpact - 10) return 'declining';
  return 'stable';
}
